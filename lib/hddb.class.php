...
class hddb {
	...
	function hddb($dbhost, $dbuser, $dbpw, $dbname = '',$dbcharset='utf8', $pconnect=0){
		$this->remember = array( $dbhost , $dbuser , $dbpw , $dbname , $dbcharset , $pconnect );
	}
	function hddb_connect($dbhost, $dbuser, $dbpw, $dbname = '',$dbcharset='utf8', $pconnect=0){
	/*
	function hddb($dbhost, $dbuser, $dbpw, $dbname = '',$dbcharset='utf8', $pconnect=0){
	*/
		if($pconnect){
	...
	function query($sql, $type = ''){
		//change by Pangee,SAE
		/* change start */
		$type = strtolower(substr(ltrim($sql), 0 , 1));
		if( ($type != 'u')  &&  ($type != 'd') && ($type != 'i') && ($type != 'r') ){
			$dbhost = SAE_MYSQL_HOST_S.':'.SAE_MYSQL_PORT;
		}else{
			$dbhost = SAE_MYSQL_HOST_M.':'.SAE_MYSQL_PORT;	
		}
		if( $dbhost != $this->remember[0] ){
			$this->remember[0] = $dbhost;
			$dba = $this->remember;
			$this->hddb_connect( $dba[0], $dba[1], $dba[2], $dba[3], $dba[4], $dba[5] );
		}
		/* change over */
		global $mquerynum;
	...
	function errorlog($msg,$sql){
		...
		file::forcemkdir("saemc://hdwiki/data/logs");
		@$fp = fopen("saemc://hdwiki/data/logs/".date('Ym')."_errorlog.php","a");
		@flock($fp, 2);
		@fwrite($fp,$error);
		@fclose($fp);
	}
