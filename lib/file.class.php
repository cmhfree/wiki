...
class file {
	...
	function readfromfile($filename) {
		if(substr($filename,0,8)=='saemc://')
		{
			return file_get_contents($filename);
		}
		if(strpos($filename,HDWIKI_ROOT)!==false)
		 {
		 ...
		 {
		 	$s = new SaeStorage();	
			return $data =  $s->read( 'hdwiki' , $filename) ; //写文件追加
		 }
		//if ($fp=@fopen($filename,'rb')) {
		//	if(PHP_VERSION >='4.3.0' && function_exists('file_get_contents')){
		//		return file_get_contents($filename);
	...
	function writetofile($filename,&$data,$type=0){
		...
		else
		{
			 ...
			 if($type)
			 {
			 	$data =  $s->read( 'hdwiki' , $filename).$data ; //写文件追加
			 }
			 return $s->write ('hdwiki',$filename, $data);
		}
	...
	echo 111;
		...
		if($url = $s->upload('hdwiki',$target, $attach['tmp_name'])){
			$result['result']=true;
			$result['url'] = $url;
			$result['msg'] ='&#19978;&#20256;&#25104;&#21151;';
		}
