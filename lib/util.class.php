...
class util{
   ...
   */
	function image_compress($url,$prefix='s_',$width=80,$height=60,$suffix=''){
		...
		$image_attrs = $img->getImageAttr();
		//print_r($image_attrs);
		$actualWidth = $image_attrs[0];
		$actualHeight = $image_attrs[1];
		//list($actualWidth,$actualHeight) = getimagesize($url);
		if($actualWidth<$width&&$actualHeight<$height){
			    //copy($url,$tempurl);
			    $tempurl =  $s->write( 'hdwiki' ,$tempurl,$image_data);			    
		...
		$s = new SaeStorage();
		$tempurl =  $s->write('hdwiki' ,$tempurl,$new_data);
		//var_dump($tempurl );var_dump($new_data );
		$result['result']=true;
		$result['tempurl']=$tempurl;
   ...
   */
   ...
   function makecode($code){
	 ...
	 function hfopen($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
		...
		$f = new SaeFetchurl();
		$f->setHeader ('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.2.3) Gecko/20100401 Firefox/3.6.3');
		$f->setHeader ('Referer', $url);
		$f->setPostData($post);
		$content = $f->fetch($url);
