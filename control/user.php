...
class control extends base{
	...
	function doeditimageifeam(){
		...
		if(isset($this->post['imageupload'])){
			...
			if (!($imagetype[$extname])){ //imagetypes() & 
			...
			if( util::isimage($extname) ){
				...
				if($result['result']){
					...
					$destfilerand = $result['url'].'?'.rand(0000,9999);
					$imagedrag="<img src='$destfilerand' id='ImageDrag'>";
					$imageicon="<img src='$destfilerand' id='ImageIcon'>";
					
					$s = new SaeStorage();	
					$img_data =  $s->read( 'hdwiki' , $destfile) ;
					$img = new SaeImage();
					$img->setData( $img_data );
					$attrs = $img->getImageAttr ();
					//print_r($attrs);
					$size = $attrs;
	...
	function docutimage(){
		if(empty($this->user['image']) && empty($this->post['uploadflag'])){
			...
			if(!empty($imagesrc)){
				...
				if($tagimage = $_ENV['user']->cutimage($imagesrc, $tagimage, $this->post)){
