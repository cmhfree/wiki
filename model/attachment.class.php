...
class attachmentmodel {
	...
	function add_attachment($uid,$did,$filename,$attachment,$description,$filetype='.jpg',$isimage=1,$coindown=0){
		//echo $attachment = 'http://'.$_SERVER['HTTP_APPNAME'].'-hdwiki.stor.sinaapp.com/'.$attachment;
		//getAttr 
		//var_dump($attachment);die();
		$s = new SaeStorage();	
		$attrs =  $s->getAttr( 'hdwiki' , $attachment) ; //д�ļ�׷��
		//print_r($attrs);
		$filesize = $attrs['length'];
		$attachment = $s->getUrl( 'hdwiki' , $attachment) ; 
