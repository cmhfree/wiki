...
class usermodel {
	...
	function cutimage($srcimage, $tagimage, $post){
		...
		$targh=90;		
		$s = new SaeStorage();	
		$img_data =  $s->read( 'hdwiki' , $srcimage) ;
		$img = new SaeImage();
		$img->setData( $img_data );
		$lx = round($post['img_x']*$post['f']/$post['width'],2);
		$rx = round(($post['img_x']+$post['img_w'])*$post['f']/$post['width'],2);
		$by = round($post['img_y']*$post['f']/$post['height'],2);
		$ty = round(($post['img_y']+$post['img_h'])*$post['f']/$post['height'],2);
		$img->crop($lx, $rx, $by ,$ty);
		$img->resize($post['dst_w'],$post['dst_h']);
		$new_data = $img->exec();
		return $img_url =  $s->write( 'hdwiki',$tagimage,$new_data) ;		
	}
